...
Note that the `Benchmark` object creates its auxiliaty HDF5 file.
...
Evaluate(
  benchmark       = b,
  seed.clustering = 1  # for exact reproducibility of results
)
```
...
-   The clustering reveals subpopulations along the *common myeloid progenitors* (CMP) to *granulocyte-monocyte progenitors* (GMP) differentiation pathway (clusters 24, 39).
    Perhaps the gating there can be refined also.

-   **Some overlaps between clusters matchable to eosinophils, classical monocytes, and intermediate monocytes is indicated.** By investigating the identity of the respective populations and mixed clusters (*e.g.*, clusters 24, 33), we can figure out further whether this is a mistake in the gating, or in the clustering.
